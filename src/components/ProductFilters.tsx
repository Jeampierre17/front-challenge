...
import { Dialog, Transition, Listbox, ListboxButton, ListboxOption, ListboxOptions, TransitionChild, DialogPanel } from '@headlessui/react';
    ...
    <div className="w-full md:px-6 xl:px-0 max-w-full md:max-w-3xl xl:max-w-7xl mx-auto">
      ...
      <Transition.Root show={showSheet} as={Fragment}>
        <Dialog as="div" className="relative z-50 xl:hidden" onClose={setShowSheet}>
          ...
          <div className="fixed inset-0 flex items-end">
            <TransitionChild as={Fragment} enter="transition ease-in-out duration-300 transform" enterFrom="translate-y-full" enterTo="translate-y-0" leave="transition ease-in-out duration-300 transform" leaveFrom="translate-y-0" leaveTo="translate-y-full">
              <DialogPanel className="w-full bg-white dark:bg-gray-900 rounded-t-2xl shadow-xl p-4 pb-4">
      ...
      <div className="hidden xl:flex sticky top-4 z-30 w-full bg-white/90 dark:bg-gray-900/90 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-lg px-2 md:px-3 xl:px-4 py-2 md:py-2.5 xl:py-3 mb-6 gap-1 md:gap-2 xl:gap-3 items-center flex-nowrap scrollbar-thin scrollbar-thumb-gray-300 dark:scrollbar-thumb-gray-700 text-sm md:text-[15px] xl:text-base">
        ...
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1 min-w-[120px]">
          ...
          <Listbox value={filters.category} onChange={handleCategoryChange}>
            <div className="relative">
              <ListboxButton className="flex items-center gap-1 bg-transparent text-sm text-gray-900 dark:text-gray-100">
                <span className="truncate">{selectedCategory.label}</span>
                <ChevronUpDownIcon className="h-4 w-4 text-gray-400 dark:text-gray-300" />
              </ListboxButton>
              <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
        ...
        <div className="flex items-center gap-2 bg-gray-100 dark:bg-gray-800 rounded-full px-3 py-1 min-w-[120px]">
          ...
          <Listbox value={filters.minRating ?? ''} onChange={handleRatingChange}>
            <div className="relative min-w-[90px]">
              <ListboxButton className="flex items-center gap-1 bg-transparent text-xs text-gray-900 dark:text-gray-100 px-2 py-1 rounded border border-gray-200 dark:border-gray-700 w-full">
                {filters.minRating ? (
              ...
              <Transition as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
                <ListboxOptions className="absolute left-0 mt-2 max-h-56 w-32 min-w-[100px] overflow-auto rounded-xl bg-white dark:bg-gray-800 py-1 text-xs shadow-2xl ring-1 ring-black/10 focus:outline-none z-[9999] border border-gray-100 dark:border-gray-700">
                  <ListboxOption value={''} className={({ active }) => `cursor-pointer select-none py-2 pl-8 pr-2 ${active ? 'bg-purple-100 text-purple-900 dark:bg-purple-900 dark:text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                    {({ selected }) => (
                      <span className={`block truncate ${selected ? 'font-semibold' : ''}`}>Todas</span>
                    )}
                  </ListboxOption>
                  {[1,2,3,4].map(val => (
                    <ListboxOption key={val} value={val} className={({ active }) => `cursor-pointer select-none py-2 pl-8 pr-2 flex items-center ${active ? 'bg-purple-100 text-purple-900 dark:bg-purple-900 dark:text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                      {({ selected }) => (
                  ...
                  <ListboxOption value={5} className={({ active }) => `cursor-pointer select-none py-2 pl-8 pr-2 flex items-center ${active ? 'bg-purple-100 text-purple-900 dark:bg-purple-900 dark:text-white' : 'text-gray-900 dark:text-gray-100'}`}>
                    {({ selected }) => (
                      <span className={`flex items-center gap-1 ${selected ? 'font-semibold' : ''}`}>
