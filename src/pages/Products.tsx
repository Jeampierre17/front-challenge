...
const Products: React.FC = () => {
  ...
  // Handler para agregar al carrito (protege contra doble ejecución)
  const [adding, setAdding] = useState(false);
  const lastToastRef = React.useRef<string | null>(null);
  const handleAddToCart = useCallback((productId: number) => {
    if (adding) return; // Evita duplicados por doble click
    setAdding(true);
    ...
    let i = 0;
    const successMsg = `Agregado al carrito: ${prod.title}`;
    if (i > 0) {
      toast.success(successMsg);
      lastToastRef.current = successMsg;
      i++;
    }
    setTimeout(() => setAdding(false), 500); // Rehabilita tras medio segundo
  ...
  return (
    <>
      ...
      >
        ...
        <div className="hidden md:block w-full mb-1 mt-2" >
          <ProductFilters
            filters={filters}
            categories={categories}
            onFiltersChange={setFilters}
        ...
        {/* Products Grid */}
        <main className="flex-1 px-0 md:px-2 xl:px-0 max-w-full md:w-full xl:w-full mx-auto">
          ...
          {isLoading ? (
            ...
            <>
              <div className="flex justify-between items-center mt-0 mb-0 ml-10">
                <p className="text-gray-600 dark:text-gray-400 font-quicksand">
              ...
              <div className="hidden md:block">
                <Suspense fallback={
                  ...
                  <AnimatePresence mode="wait">
                    ...
                    <motion.div
                      key="product-grid" // asegura animación en cambio de key (opcional pero útil en paginación)
